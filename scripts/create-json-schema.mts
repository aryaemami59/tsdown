...
/**
 * Parses a whole-module import type - `typeof import("some-module")` - into a
 * {@linkcode NeverType}, which drops the property carrying it from the
 * generated schema.
 *
 * `ImportTypeNodeParser` resolves an
 * {@linkcode ts.ImportTypeNode | import type} without a
 * {@linkcode ts.ImportTypeNode.qualifier | qualifier} to the declaration of
 * the module itself, which is a {@linkcode ts.SourceFile}. No built-in parser
 * handles that node kind, so `ChainNodeParser` throws
 * `UnknownNodeError: Unknown node of kind "SourceFile"`.
 *
 * A module namespace object is not expressible in JSON anyway, so omitting
 * such properties is the correct outcome for a JSON schema. Only optional
 * properties are dropped - a _required_ one makes its containing type
 * unrepresentable, which `InterfaceAndClassNodeParser` reports on its own.
 *
 * @see {@link https://github.com/vega/ts-json-schema-generator/blob/main/src/NodeParser/ImportTypeNodeParser.ts | `ImportTypeNodeParser` source}
 */
const sourceFileNodeParser = {
  createType: () => new NeverType(),

  supportsNode: (node: ts.Node): node is ts.SourceFile =>
    node.kind === ts.SyntaxKind.SourceFile,
} as const satisfies SubNodeParser

const augmentParser = ((parser) => {
  parser.addNodeParser(sourceFileNodeParser)
}) satisfies ParserAugmentor
...
const objectConfig = Object.values(entries).map((entry) => {
  ...
  const parser = createParser(tsProgram, schemaGeneratorConfig, augmentParser)
