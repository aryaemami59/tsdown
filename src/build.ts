...
/**
 * Build a TypeScript/JavaScript library with `tsdown`. The programmatic API
 * accepts the same options as the config file and the CLI. This is useful for
 * custom build scripts, integrations, or advanced automation.
 *
 * @example
 ...
 * ```
 *
 * @param [inlineConfig] - Options to pass inline, equivalent to a config file. Defaults to `{}`.
 * @returns An array of {@linkcode TsdownBundle} objects - one per resolved config (i.e. one per output format X workspace package).
 *
...
/**
 * Build with pre-resolved configs. For internal use by
 * {@linkcode build | build()} and the CLI; prefer {@linkcode build | build()}
...
/**
 * Run a single resolved config through the full build pipeline (or start a
 * watcher for it). Does not manage shortcuts or multi-config coordination.
 * That is the responsibility of {@linkcode buildWithConfigs()}.
 *
