 ...
 * - `false`: Disabled.
 * - `true`: Separate `.map` files alongside the output, each referenced by a `//# sourceMappingURL` comment.
 * - `'inline'`: Source map embedded as a base64 data URL inside the output file. No separate `.map` file is generated.
 * - `'hidden'`: Separate `.map` files without `//# sourceMappingURL` comments.
 *
 * @see {@link https://tsdown.dev/options/sourcemap | sourcemap option documentation}
 */
export type Sourcemap = boolean | 'inline' | 'hidden'

/**
 * Accepted output format identifiers for `tsdown`. Includes canonical forms
 * (`'es'`, `'cjs'`, `'iife'`, `'umd'`) and their common aliases (`'esm'`,
 * `'module'`, `'commonjs'`).
 *
 * @see {@link https://tsdown.dev/options/output-format | output-format option documentation}
 */
export type Format = ModuleFormat

/**
 * The canonical, post-normalization output {@linkcode Format}.
 */
...
/**
 * A toggleable option that can be a plain `boolean`, a {@linkcode CIOption}
 * string, or a configuration object for the feature that also accepts an
 * `enabled` field.
 *
 * | Value          | Behavior                              |
 * | -------------- | -------------------------------------- |
...
export interface UserConfig {
   ...
   *
   * @example
   * <caption>Supports [glob patterns][glob-patterns], enabling you to match multiple files dynamically.</caption>
   *
   * ```ts
   * import { defineConfig } from 'tsdown';
   *
   ...
   *
   * @example
   * <caption>Supports [glob patterns][glob-patterns] in arrays, with negation patterns to exclude specific files.</caption>
   *
   * ```ts
   * import { defineConfig } from 'tsdown';
   *
   ...
   *
   * @example
   * <caption>You can mix strings, [glob patterns][glob-patterns], and object entries in an array</caption>
   *
   * ```ts
   * import { defineConfig } from 'tsdown';
   *
   ...
   *
   * - `'node'`: Targets the {@link https://nodejs.org | Node.js} runtime and compatible environments such as Deno and Bun. This is the default platform, and Node.js built-in modules (e.g., `fs`, `path`) will be resolved automatically. Ideal for toolchains or server-side projects. For CJS format, this is always set to `'node'` and cannot be changed.
   ...
   *
   * Determines the JavaScript version or runtime(s) for which the code should
   * be compiled. If not set, defaults to the value of `engines.node` in your
   * project's `package.json`. If no `engines.node` field exists, no syntax
   ...
   * ```
   *
   * @see {@link https://tsdown.dev/options/target#supported-targets | Supported Targets} for a list of valid targets and more details.
   * @see {@link https://tsdown.dev/options/target | `target` option documentation} for more details.
   */
   ...
   *
   * @default true
   * @see {@link https://tsdown.dev/options/tree-shaking | `tsdown` documentation}
   * @see {@link https://rolldown.rs/reference/InputOptions.treeshake | Rolldown documentation} for more details.
   */
   ...
   * {@linkcode UserConfig.watch | watch} mode.
   *
   * @default true
   */
  ...
  /**
   * Clean directories before build. Default to
   * {@linkcode UserConfig.outDir | output directory}.
   *
   * @default true
   */
   ...
   * {@linkcode https://oxc.rs/docs/guide/usage/minifier | oxc-minify},
   * which removes whitespace, comments, and renames variables to reduce
   * bundle size.
   *
   * @default false
   ...
   * {@linkcode UserConfig.format | format} is set to `'cjs'`, `tsdown` will
   ...
   * When using {@linkcode UserConfig.workspace | workspace} mode, this option defaults to the
   ...
   *
   * @default 'info'
   * @see {@link https://tsdown.dev/options/log-level | `logLevel` option documentation} for more details.
   */
  logLevel?: LogLevel
   ...
   * predicate function. Matched warnings are dropped before
   * {@linkcode UserConfig.failOnWarn | failOnWarn} is applied, so they won't
   * fail the build.
   */
  ...
  /**
   * Files or patterns to not watch while in
   * {@linkcode UserConfig.watch | watch} mode.
   *
   * @default []
   */
  ...
  /**
   * You can specify command to be executed after a successful build,
   * especially useful for {@linkcode UserConfig.watch | watch} mode.
   */
  onSuccess?:
    string | ((config: ResolvedConfig, signal: AbortSignal) => Awaitable<void>)
   ...
   * - If the {@linkcode PackageJsonWithPath.types | types} or `typings` field is present in `package.json`, or if its {@linkcode PackageJsonWithPath.exports | exports} field contains a `types` entry, declaration file generation is enabled by default.
   ...
   * is enabled in `tsconfig.json`. With
   * {@linkcode https://www.typescriptlang.org/tsconfig/#isolatedDeclarations | isolatedDeclarations},
   * declarations are generated via
   * {@linkcode https://oxc.rs/docs/guide/usage/transformer | oxc-transform}
   * (very fast). Without it, the TypeScript compiler is used (slower).
   *
   ...
   * {@linkcode PackageJsonWithPath.exports | exports} field in `package.json`
...
export interface InlineConfig extends UserConfig {
   ...
   * - `'tsx'`: Loads configuration files using the {@linkcode https://tsx.hirok.io | tsx} library via its {@linkcode https://tsx.hirok.io/dev-api/ts-import | tsImport} API. Note that {@linkcode https://tsx.hirok.io | tsx} is an optional peer dependency. You need to install it manually if you want to use this loader.
...
/**
 * The type for a function that generates user configuration objects. This
 * function receives the inline configuration and a context object, and returns
 * one or more {@linkcode UserConfig | configuration objects}.
 */
export type UserConfigFn =
  /**
   * A function that generates {@linkcode UserConfig | configuration objects}
   * based on the provided {@linkcode InlineConfig | inline configuration} and
   * {@linkcode context}. This allows for dynamic configuration that can adapt
   * to different environments or inputs. The `context.ci` boolean lets you
   * branch on whether the build is running inside a CI environment.
   *
   * @example
   ...
   * ```
   *
   * @param inlineConfig - The {@linkcode InlineConfig | inline configuration}, such as CLI flags. This is useful for dynamic configuration based on CLI input.
   * @param context - An object providing additional context for configuration.
   * @returns One or more {@linkcode UserConfig | config} objects, or a {@linkcode Promise | promise} that resolves to one or more {@linkcode UserConfig | config} objects.
   */
  (
    inlineConfig: InlineConfig,
    context: {
      /**
       * A `boolean` indicating whether the build is running in a CI
       * environment.
       *
       * @see {@link https://tsdown.dev/advanced/ci | CI environment documentation}
       */
      ...
      /**
       * The root configuration object if this function is being called for a
       * {@linkcode UserConfig.workspace | workspace} config; otherwise,
       * `undefined`.
       */
 ...
 * - A single {@linkcode UserConfig | config} object.
 * - An array of {@linkcode UserConfig | config} objects (for multiple outputs).
 * - A {@linkcode UserConfigFn | config function} function.
 * - A {@linkcode Promise | promise} resolving to any of the above.
 *
 * @example
...
export type ResolvedConfig = Overwrite<
  ...
  >,
  {
     ...
     * (for {@linkcode UserConfig.watch | watch} mode re-globbing).
     */
