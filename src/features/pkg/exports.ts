...
export interface ExportsOptions {
   ...
   * - `true`: all conditions point to source files, and add `dist` exports to {@linkcode PackageJson.publishConfig | publishConfig}.
   */
  ...
  /**
   * Specifies file patterns (as glob patterns or regular expressions) to
   * exclude from package exports. Use this to prevent certain files from being
   * included in the exported package, such as test files, binaries, or
   * internal utilities.
   *
   * **Note:** Do not include file extensions, and paths should be relative to
   * the `dist` directory.
   *
   * @example
  ...
  /**
   * Generate legacy fields ({@linkcode PackageJson.main | main},
   * {@linkcode PackageJson.module | module} and
   * {@linkcode PackageJson.types | types}) for older Node.js versions and
   * bundlers that do not support the package
   * {@linkcode PackageJson.exports | exports} field.
   *
  ...
  /**
   * Specifies custom exports to add to the package exports in addition to the
   * ones generated by tsdown. Use this to add additional exports in the
  ...
  /**
   * Generate `inlinedDependencies` field in `package.json`. Lists dependencies
   * that are physically inlined into the bundle with their exact versions.
   *
   * @default true
  ...
  /**
   * Generate the {@linkcode PackageJson.bin | bin} field in `package.json`
   * for CLI executables.
   *
   ...
   *   contain a shebang.
   *
   * When {@linkcode ExportsOptions.devExports | .devExports} is enabled, the
   * {@linkcode PackageJson.bin | bin} field in `package.json` points to source
   * files during local development, while `publishConfig.bin` points to built
   * output paths for publishing.
   *
   * @example
 ...
 * @param options - Subset of {@linkcode ResolvedConfig | resolved config} fields that influence export generation.
 * @param [inlinedDeps] - Map of inlined dependency names to their bundled versions, included in the output for tooling consumers.
 * @returns An object containing the computed `exports`, legacy fields, and other metadata without writing to disk.
 */
...
function exportMeta(
  exports: Record<string, any>,
  all?: ExportsOptions['all'],
  packageJson?: ExportsOptions['packageJson'],
) {
  if (all) {
    exports['./*'] = './*'
  } else if (packageJson) {
