...
/**
 * Log severity levels used internally. Excludes `'silent'`, which is a
 * log-level threshold rather than an actual message type.
 */
export type LogType = 'error' | 'warn' | 'info'

/**
 * Accepted values for the logger's threshold level.
 *
...
export interface LoggerOptions {
   ...
   * {@linkcode LoggerOptions.failOnWarn | failOnWarn} is applied, so they
...
/**
 * The resolved (fully-populated) form of {@linkcode LoggerOptions}, produced
 * internally by {@linkcode createLogger()} after applying defaults.
 */
...
/**
 * Numeric priority values for each {@linkcode LogLevel}. A higher number
 * means more verbose output.
 *
 * | Level    | Value |
 * | -------- | ----- |
 * | `silent` | `0`   |
...
export interface Logger {
  ...
  /**
   * Log an error message. These messages indicate issues that prevent
   * the build from completing successfully. After logging an error, the
   * process will exit with a non-zero code.
   */
  error: (...msgs: any[]) => void
   ...
   * `warn`.
   *
   * @param type - The {@linkcode LogType | log type} that determines whether the screen should be cleared based on the logger's current {@linkcode LogLevel | log level}.
   */
 ...
 * logger provides methods for logging messages at different
 * {@linkcode LogLevel | log levels} and can be customized with options such as
 * allowing screen clearing, providing a custom logger implementation, and
 * treating warnings as errors.
 *
 * @param [level] - The {@linkcode LogLevel | log level} for the logger. Messages with a severity below this level will not be logged. The levels are ordered as follows: `'silent'` (0), `'error'` (1), `'warn'` (2), and `'info'` (3). Default is `'info'`.
...
/**
 * The default global {@linkcode Logger} instance, created with log level
 * `'info'` and default options.
 */
 ...
 *
 * @param format - The Rolldown {@linkcode InternalModuleFormat | internal module format} to label.
 ...
 *
 * @param [name] - Identifier whose color to retrieve or generate. Defaults to `'default'`.
